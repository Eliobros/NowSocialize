...
export async function GET(
  ...
  try {
    ...
    if (!ObjectId.isValid(params.pageId)) {
      return NextResponse.json({ error: "ID da página inválido" }, { status: 400 });
    }

    const pageId = new ObjectId(params.pageId);
...
export async function PUT(
  ...
  try {
    ...
    await pages.updateOne({ _id: pageId }, { $set: updateData });

    const updatedPage = await pages.findOne({ _id: pageId });
